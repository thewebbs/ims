#------------------------------------------------------------
# Filename  : agent_action_exc_rate_todos.py
# Project   : ava
#
...
# Date       Ver Who Change
# ---------- --- --- ------
# 2020-05-20   1 MW  Initial write based on agent_action_histmktdata_todos
# ...
# 2021-12-19 100 DW  Added version 
# 2021-12-27 101 MW  Changed start_date to start_datetime for
#                    both get_load_todo and set_todo_status
...
from database.db_objects.ImsExchangeRateDataDB import ImsExchangeRateDataDB
from database.db_objects.ImsInvestmentDB import update_latest_price_date
from database.db_objects.ImsCurrLoadDoneDB import ImsCurrLoadDoneDB
from database.db_objects.ImsCurrLoadTodoDB import delete_curr_load_todo, get_curr_load_todo, set_todo_status
...
def SetupLogger():
    ...
    logging.basicConfig(filename = time.strftime("log/Action_Exc_Rate_Todos_api.%y%m%d_%H%M%S.log"),
                        filemode = "w",
                        level    = logging.ERROR,
                        format   = recfmt, 
                        datefmt  = timefmt)
...
class TestApp(TestWrapper, TestClient):
    def __init__(self):
        ...
        self.progress_store = {}
        self.tracking_store = {}
        self.ims_curr_load_todos = {}
        
        self.erd_cty_symbol1          = ''
        self.erd_cty_symbol2          = ''
        self.erd_start_datetime       = ''
        self.erd_end_datetime         = ''
        self.erd_freq_type            = ''
        self.erd_close_price          = ''
              
        self.df_ims_exchange_rate_data = pd.DataFrame()
    ...
    def start(self):
        ...
        if self.globalCancelOnly:
            ...
            result = set_todo_status(database           = DB,
                                     ...
                                     clt_freq_type      = '%', 
                                     clt_start_datetime = '%', 
                                     old_clt_status     = oldStatus, 
                                     new_clt_status     = newStatus)
            print(' ')
            print('Set WIP CURR_LOAD_TODOS records to RDY')   
    ...
    def error(self, reqId: TickerId, errorCode: int, errorString: str):
        ...
        if (errorCode != 2104) and (errorCode != 2106) and (errorCode != 2107) and (errorCode != 366):
            ...
            if (errorCode == 200) or (errorCode == 162) or (errorCode == 165):
                ...
                this_erd_id = self.progress_store[reqId][0]
                error_message = 'Error ' + str(errorCode) + ' for request id ' + str(this_erd_id)
                print(error_datetime, error_message)
                                
                # get the pk info for this erd_id
                erd_cty_symbol1 = this_erd_id.split('$')[0]
                erd_cty_symbol2 = this_erd_id.split('$')[1]
                erd_freq_type   = this_erd_id.split('$')[2]
                erd_start_date = this_erd_id.split('$')[3]
                erd_end_date   = this_erd_id.split('$')[4]
                
                oldStatus = 'WIP'
                                     
                result = set_todo_status(database            = DB,
                                         clt_cty_symbol1     = erd_cty_symbol1, 
                                         clt_cty_symbol2     = erd_cty_symbol2, 
                                         clt_freq_type       = erd_freq_type, 
                                         clt_start_datetime  = erd_start_date, 
                                         old_clt_status      = oldStatus, 
                                         new_clt_status      = errorCode)
                
                print(error_datetime, 'Set this IMS_CURR_LOAD_TODOS record to In Error code ',str(errorCode), erd_cty_symbol1, erd_cty_symbol2, erd_freq_type, erd_start_date)   
                
                print(error_datetime, 'About to cancel reqid ',reqId, ' for ',erd_cty_symbol1, erd_cty_symbol2, erd_freq_type, erd_start_date)
                ...
                print(error_datetime, 'Cancelled reqid ', reqId, ' for ', erd_cty_symbol1, erd_cty_symbol2, erd_freq_type, erd_start_date)
                
                the_message = '%s * Dataload request cancel for request %3.0f for ticker %s type %s %s for date %s ' % (error_datetime, reqId, erd_cty_symbol1, erd_cty_symbol2, erd_freq_type, erd_start_date)
                
                print(error_datetime, 'Creating a new replacement request')
                self.historicalDataOperations_req()
    ...
    def historicalDataOperations_req(self):
        ...
        self.number_active_requests += 1
        # Need to work out the frequency type, duration and queryTime from items in the ims_curr_load_todos table 
        # we will only return one request from get_curr_load_todo

        todo_list = get_curr_load_todo(database    = DB,
                                  clt_cty_symbol1  = '%', 
                                  clt_cty_symbol2  = '%', 
                                  clt_freq_type    = '%', 
                                  clt_start_date   = '%', 
        ...
        if todo_list != []:
            ...
            for clt_cty_symbol1, clt_cty_symbol2, clt_freq_type, clt_start_date, clt_end_date in todo_list:
                ...
                print('****** Starting Exchange Rate DataLoad- ', time.strftime('%l:%M:%S on %b %d, %Y'),' for ', clt_cty_symbol1, clt_cty_symbol2, clt_freq_type, clt_start_date, clt_end_date)
                
                # common stuff for all types
                    
                num_days           = clt_end_date - clt_start_date
                self.req_duration  = str(num_days.days)  + ' D'
                self.erd_freq_type = clt_freq_type
                queryTime          = datetime.datetime.strftime(clt_end_date,"%Y%m%d %H:%M:%S")
                    
                fmt_start_date           = datetime.datetime.strftime(clt_start_date,"%Y%m%d%H%M%S")
                fmt_end_date             = datetime.datetime.strftime(clt_end_date,"%Y%m%d%H%M%S")
                erd_id                   = "%s$%s$%s$%s$%s" % (clt_cty_symbol1, clt_cty_symbol2, clt_freq_type, clt_start_date, clt_end_date)
                ...
                if num_days.days > 0:
                    ...
                    self.reqHistoricalData(self.req_id, ContractSamples.ExcFx(clt_cty_symbol1, clt_cty_symbol2), queryTime, 
                                                   self.req_duration, self.erd_freq_type, req_type, 1, 1, keepUpToDate, chartOptions)
                    new_dict1 = {self.req_id : [erd_id, False, self.erd_freq_type]}
                    self.progress_store.update(new_dict1)
                    
                    original_tracking_list = self.tracking_store.get(erd_id)
                    new_list               = []
                    
                    if original_tracking_list == None:
                        new_list = {erd_id : [self.req_id]}
                    else:
                        original_tracking_list.append(self.req_id)
                        new_list = {erd_id :original_tracking_list} 
                    ...
                    newStatus = 'WIP'
                    result = set_todo_status(database            = DB,
                                             clt_cty_symbol1     = clt_cty_symbol1, 
                                             clt_cty_symbol2     = clt_cty_symbol2, 
                                             clt_freq_type       = clt_freq_type, 
                                             clt_start_datetime  = clt_start_date, 
                                             old_clt_status      = oldStatus, 
                                             new_clt_status      = newStatus) 
    
        else:
            print('***************************************************************************************************************')
            print('***** Went to load_todos and found no Curr Load Todo requests waiting to be processed so now we should end *****')
    ...
    def historicalData(self, reqId:int, bar: BarData):
        ...
        erd_id = this_tracking_rec[0]
        
        # change delimiter to dollar sign rather than period
        erd_cty_symbol1 = erd_id.split('$')[0]
        erd_cty_symbol2 = erd_id.split('$')[1]
        erd_freq_type   = erd_id.split('$')[2]
        
        erd_start_datetime = (datetime.datetime.strptime(bar.date,"%Y%m%d  %H:%M:%S")).strftime("%Y-%m-%d %H:%M:%S")
        index_start_dt = (datetime.datetime.strptime(bar.date,"%Y%m%d  %H:%M:%S")).strftime("%Y%m%d%H%M")
        
        if self.erd_freq_type == '1 min':
            erd_end_datetime = (datetime.datetime.strptime(bar.date,"%Y%m%d %H:%M:%S") + datetime.timedelta(seconds=59)).strftime("%Y-%m-%d %H:%M:%S")
            index_end_dt = (datetime.datetime.strptime(bar.date,"%Y%m%d %H:%M:%S") + datetime.timedelta(seconds=59)).strftime("%Y%m%d%H%M")
        elif self.erd_freq_type == '5 mins':
            erd_end_datetime = (datetime.datetime.strptime(bar.date,"%Y%m%d %H:%M:%S") + datetime.timedelta(seconds=59)).strftime("%Y-%m-%d %H:%M:%S")
            index_end_dt = (datetime.datetime.strptime(date,"%Y%m%d %H:%M:%S") + datetime.timedelta(seconds=299)).strftime("%Y%m%d%H%M")
        elif self.erd_freq_type == '15 mins':
            erd_end_datetime = (datetime.datetime.strptime(bar.date,"%Y%m%d %H:%M:%S") + datetime.timedelta(seconds=59)).strftime("%Y-%m-%d %H:%M:%S")
        ...
        # change delimiter to dollar sign rather than period
        full_index_col = erd_cty_symbol1 + '$' + erd_cty_symbol1 + '$' + index_start_dt + '$' + index_end_dt
        
        erd_close_price    = bar.close
        
        # Don't know whether already exists in data frame or not so check first
            
        if len(self.df_ims_exchange_rate_data.index) > 0:
                
            if full_index_col in self.df_ims_exchange_rate_data.index:
                    
                # already exists so just update 
                    
                self.df_ims_exchange_rate_data.at[full_index_col,'erd_close_price']    = erd_close_price      
                ...
                this_dict = {'full_index_col':       [full_index_col],
                             'erd_id':               [erd_id],
                             'erd_cty_symbol1':      [erd_cty_symbol1], 
                             'erd_cty_symbol2':      [erd_cty_symbol2], 
                             'erd_start_datetime':   [erd_start_datetime], 
                             'erd_end_datetime':     [erd_end_datetime], 
                             'erd_freq_type':        [erd_freq_type], 
                             'erd_close_price':      [erd_close_price]
                            }
                
                df_temp = pd.DataFrame.from_dict(this_dict)
                df_temp.set_index("full_index_col", inplace=True)
                self.df_ims_exchange_rate_data = pd.concat([self.df_ims_exchange_rate_data, df_temp], axis = 0, sort=False)
    ...
    def historicalDataEnd(self, reqId: int, start: str, end: str):
        ...
        these_requests = self.tracking_store[this_erd_id]
        ...
        # if all requests for this currency symbol combo have completed then can save to the database
        
        if any_requests_os == False:
            
            print('****** About to save - ', time.strftime('%l:%M:%S on %b %d, %Y'),' for',this_erd_id)   
            
            for index, thisrow in self.df_ims_exchange_rate_data.iterrows():
                
                # only prepare the record if it matches the ticker/exchange symbol - ignore the others
                df_erd_id = thisrow.erd_id
                
                if df_erd_id == this_erd_id:
                    

                    this_exchange_rate = ImsExchangeRateDataDB(database           = DB,
                                                               erd_cty_symbol1    = thisrow.erd_cty_symbol1, 
                                                               erd_cty_symbol2    = thisrow.erd_cty_symbol2, 
                                                               erd_start_datetime = thisrow.erd_start_datetime, 
                                                               erd_end_datetime   = thisrow.erd_end_datetime, 
                                                               erd_freq_type      = thisrow.erd_freq_type, 
                                                               erd_close_price    = thisrow.erd_close_price)
                    result = this_exchange_rate.select_DB()
                    
                    if result == None:
                        this_exchange_rate.insert_DB()
                    else:
                        this_exchange_rate.update_DB()     
                        
                        
                    # now remove from the data frame rows for this ticker/exchange symbol
                    self.df_ims_exchange_rate_data.drop(index, inplace = True) 
                
                    
            # get the pk info for this erd_id
            cld_cty_symbol1 = this_erd_id.split('$')[0]
            cld_cty_symbol2 = this_erd_id.split('$')[1]
            cld_freq_type   = this_erd_id.split('$')[2]
            this_start_date = this_erd_id.split('$')[3]
            this_end_date   = this_erd_id.split('$')[4]
            
            #cld_start_date = datetime.datetime.strptime(this_start_date,"%Y%m%d%H%M%S").strftime("%Y-%m-%d %H:%M:%S")
            #cld_end_date = datetime.datetime.strptime(this_end_date,"%Y%m%d%H%M%S").strftime("%Y-%m-%d %H:%M:%S")
            cld_start_date = this_start_date
            cld_end_date = this_end_date
            cld_date_loaded = datetime.datetime.today().strftime("%Y-%m-%d %H:%M:%S")
            
            load_done_data = ImsCurrLoadDoneDB(database        = DB,
                                               cld_cty_symbol1 = cld_cty_symbol1,
                                               cld_cty_symbol2 = cld_cty_symbol2,
                                               cld_freq_type   = cld_freq_type,
                                               cld_start_date  = cld_start_date,
                                               cld_end_date    = cld_end_date,
                                               cld_date_loaded = cld_date_loaded
                                               )
            ...
            delete_curr_load_todo(database       = DB,
                                  clt_cty_symbol1 = cld_cty_symbol1,
                                  clt_cty_symbol2 = cld_cty_symbol2,
                                  clt_freq_type   = cld_freq_type,
                                  clt_status      = 'WIP',
                                  clt_start_date  = cld_start_date
                                  )
            
            
            print('****** Finished writing - ', time.strftime('%l:%M:%S on %b %d, %Y'),' Saved to loaded_data & removed from curr_data_todos for',this_erd_id)       
            print(' ')
...
def agent_action_exc_rate_todos():
    SetupLogger()
...
if __name__ == "__main__":
    ...
    print("Started agent_action_exc_rate_todos")
    print(" ")
    print("Open db")
    print(" ")
