#------------------------------------------------------------
# Filename  : agent_action_earning_todos.py
# Project   : ava
#
# Descr     : This holds routines relating to the IB API to get fundamentals data
#             This creates records in ims_earnings
#
# Params    : None
#
# History   :
...
# Date       Ver Who Change
# ---------- --- --- ------
# 2020-01-25   1 MW  Initial write based on TWS Program.py
# ...
# 2021-12-19 100 DW  Added version 
# 2021-12-27 101 MW  Changed start_date to start_datetime for
#                    get_load_todo and set_todo_status
#                    and delete_load_todo
...
import pandas as pd
from database.db_objects.ImsEarningDB import ImsEarningDB
...
from utils.config import IB_EARNING_CLIENTID
...
def SetupLogger():
    ...
    logging.basicConfig(filename=time.strftime("log/TWS_earnings_api.%y%m%d_%H%M%S.log"),
                        filemode="w",
                        level=logging.ERROR,
...
class TestApp(TestWrapper, TestClient):
    ...
    def start(self):
        ...
        if self.globalCancelOnly:
            ...
            print(' ')
            print('Set WIP EARN ims_load_todos record to RDY')   
    ...
    def error(self, reqId: TickerId, errorCode: int, errorString: str):
        ...
        if (errorCode != 2104) and (errorCode != 2106) and (errorCode != 2107) and (errorCode != 366):
            ...
            if (errorCode == 200) or (errorCode == 162) or (errorCode == 165) or (errorCode == 430):
                ...
                this_ear_id = self.progress_store[reqId][0]
                error_message = 'Error ' + str(errorCode) + ' for request id ' + str(this_ear_id)
                print(error_datetime, error_message)
                                    
                # get the pk info for this ear_id
                inv_ticker      = this_ear_id.split('$')[0]
                freq_type       = this_ear_id.split('$')[1]
                req_type        = this_ear_id.split('$')[2]
                this_start_date = this_ear_id.split('$')[3]
                this_end_date   = this_ear_id.split('$')[4]
                ...
                the_message = '%s * ear Dataload request cancel for request %3.0f for ticker %s type %s %s for date %s ' % (error_datetime, reqId, inv_ticker, freq_type, req_type, this_start_date)
                
                print(error_datetime, 'Creating a new replacement request (ear)')
                self.fundamentalsOperations_req()
    ...
    def fundamentalsOperations_req(self):
        ...
        if todo_list != []:
            ...
            for inv_ticker, freq_type, req_type, start_date, end_date in todo_list:
                ...
                print('****** Starting Earnings DataLoad- ', time.strftime('%l:%M:%S on %b %d, %Y'),' for ',inv_ticker, freq_type, req_type, start_date, end_date)
                ...
                ear_id                   = "%s$%s$%s$%s$%s" % (inv_ticker, freq_type, req_type, start_date, end_date)
                ...
                if exchange == "TO":
                    ...
                    new_dict1 = {self.req_id : [ear_id, 'EAR', False, self.freq_type]}
                ...
                original_tracking_list = self.tracking_store.get(ear_id)
                new_list = []
                if original_tracking_list == None:
                    new_list = {ear_id : [self.req_id]}
                else:
                    original_tracking_list.append(self.req_id)
                    new_list = {ear_id :original_tracking_list} 
            ...
            print('***************************************************************************************************************')
            print('********** Went to load_todos and found no EARN requests waiting to be processed so now we should end **********')
    ...
    def fundamentalData(self, reqId: TickerId, data: str):
        ...
        ear_id = this_tracking_rec[0]
        
        # change delimiter to dollar sign rather than period
        
        inv_ticker = ear_id.split('$')[0]
        freq_type  = ear_id.split('$')[1]
        req_type   = ear_id.split('$')[2]
        start_date = ear_id.split('$')[3]
        end_date   = ear_id.split('$')[4]
                
        # amended this section 22-01-2012 to save ALL earidend types        
        ...
        for child1 in root:
            ...
            if child1.tag == 'EPSs':
                ...
                for child2 in child1:
                    ...
                    earningsPerShare = child2.text
                    
                    new_ims_earning = ImsEarningDB(database        = DB,
                                                   ear_inv_ticker  = inv_ticker, 
                                                   ear_report_type = reportType, 
                                                   ear_as_of_date  = asofDate, 
                                                   ear_period      = period, 
                                                   ear_per_share   = earningsPerShare)
                    
                    new_ims_earning.insert_DB()
        ...
        print('****** Finished writing - ', time.strftime('%l:%M:%S on %b %d, %Y'),' Saved to loaded_data & removed from data_todos for',ear_id)       
        print(' ')
...
def agent_action_earning_todos():
    SetupLogger()
    ...
    try:
        ...
        app.connect(IB_API_GATEWAY_IP, the_port, clientId=IB_EARNING_CLIENTID)
        # ! [connect]
...
if __name__ == "__main__":
    ...
    print("Started agent_action_earning_todos")
    print(" ")
    print("Open db")
    print(" ")
